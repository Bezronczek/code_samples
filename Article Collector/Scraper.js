...
function _getText(html, config) {
  ...
  if (typeof elementsToInclude !== 'undefined') {
    content = article.find(elementsToInclude.join(', ')).text().trim();
  } else {
...
function scrapeToFile(dataObj, callback) {
  // outputFrame.clear();
  let config = getConfigFromURL(dataObj.url);

  // false = no config found
  if (config === false) {
    outputFrame.send(`No config found for: ${dataObj.url}`);
    return;
  }
  ...
  let outputData = {};
  _loadPage(dataObj.url, html => {
    outputData.article = _getText(html, config);
    outputData.multimedia = _detectVideo(html, config);
    _saveToFile(outputData, dataObj.file, dataObj.mm);
    callback();
    mainWindow.send('scraper-link-finished', dataObj.url);
  }, config);
}



exports = module.exports = scrapeToFile;
